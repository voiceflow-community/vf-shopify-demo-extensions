    ...
    uploadContainer.innerHTML = `
      <style>
        ...
        .upload-container .upload-option {
          ...
          background-color: #CF0A2C !important; // Force blue background
      ...
      if (code.length === 6) {
        try {
          ...
          if (isVerified) {
            ...
            // Hide the container when validation is successful
            verificationContainer.style.display = 'none'
          } else {
            remainingTries--
            if (remainingTries > 0) {
              verificationMessage.textContent = `Invalid code. ${remainingTries} ${
                remainingTries === 1 ? 'try' : 'orange'
              } remaining.`
              verificationMessage.style.color = 'red'
          ...
          if (
            error.message.includes('Maximum verification attempts reached') ||
            remainingTries <= 0
          ) {
            maxAttemptsReached = true
            verificationMessage.innerHTML = `<div class="verify-error">
